...
const Sidebar = ({ expand, setExpand }) => {
  ...
  return (
    <div className={`flex flex-col justify-between bg-[#212327] pt-7 transition-all z-50 max-md:absolute max-md:h-screen ${expand ? "p-4 w-64" : "md:w-20 w-0 max-md:overflow-hidden"} h-screen`}>
      
      {/* === Top Header === */}
      <div>
        <div className={`flex ${expand ? "flex-row gap-10" : "flex-col items-center gap-8"}`}>
          <Image className={`${expand ? "w-36" : "w-10"}`} src={expand ? logo_text : logo_icon} alt="logo" width={144} height={40} />
      ...
      <div className="flex flex-col mt-10 gap-4 overflow-y-auto flex-grow">
        {/* New Chat */}
        <button className={`flex items-center justify-center cursor-pointer ${expand ? "bg-primary hover:opacity-90 rounded-2xl gap-2 p-2.5 w-max" : "group relative h-9 w-9 mx-auto hover:bg-gray-500/30 rounded-lg"}`}>
          <Image className={expand ? "w-6" : "w-7"} src={expand ? assets.chat_icon : assets.chat_icon_dull} alt="new chat" />
        ...
        {expand && (
          <div className="text-white/25 text-sm mt-4">
            ...
            <RecentTabs openMenu={openMenu} setOpenMenu={setOpenMenu}/>
          </div>
        )}
      </div>

      {/* === Bottom Fixed GET APP Section === */}
      <div className={`relative group ${expand ? "p-2.5 border-t border-white/10" : "py-2"}`}>
        <div className={`flex items-center ${expand ? "gap-2 text-white/80 text-sm hover:bg-white/10 rounded-lg p-2" : "h-10 w-10 mx-auto hover:bg-gray-500/30 rounded-lg justify-center"}`}>
          <Image
        ...
        {/* Tooltip */}
        <div className={`absolute z-10 ${expand ? "left-full ml-2" : "-right-32"} bottom-10 opacity-0 group-hover:opacity-100 transition-all duration-300`}>
      ...
      <div onClick={user ? null: openSignIn}
       ...
       className={`flex items-center ${expand? "hover:text-white hover:bg-gray-500/30 rounded-lg": "justify-center w-full"}gap-3 text-white/60 text-sm p-2 mt-2 cursor-pointer`}>
        {
          user ? <UserButton/> : <Image src={assets.profile_icon} alt="profile" className='w-7' width={20} height={20}/>
        }
